...
/**
 * Returns a distance beetween two points by   coordinates.
 *
 * @param {number} x1
 * @param {number} y1
 * @param {number} x2
...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  let res = ((x1*x2)+(y1*y2))/Math.sqrt(Math.pow(x1,2) + Math.pow(y1,2))*Math.sqrt(Math.pow(x2,2) + Math.pow(y2,2));
 ...
 */
function isPrime(n) {
  let sqrOfn = Math.round(Math.sqrt(n));
  for(let i=2; i<=sqrOfn; i++){
